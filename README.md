...
class LoginOrSignup:
    def __init__(self) -> None:
        ...
        label1 = Label(self.root, text="Don't have an account?", font=('Verdana', 13), bg=bg_color, fg='black')
        label1.pack(pady=20)

        register_button = Button(self.root, text="Register", width=20, height=3, command=self.show_register, bg=button_color, fg='white', font=('Helvetica', 15, 'bold'))
        register_button.pack(padx=20, pady=20, ipadx=10, ipady=10)

        label2 = Label(self.root, text="Already have an account?", font=('Verdana', 13), bg=bg_color, fg='black')
        label2.pack(pady=20)

        login_button = Button(self.root, text="Login", command=self.show_login, width=20, height=3, bg=button_color, fg='white', font=('Helvetica', 15, 'bold'))
        login_button.pack(padx=20, pady=20, ipadx=10, ipady=10)
...
class LoginPage:
    def __init__(self) -> None:
        ...
        self.login = Tk()
        self.login.geometry('1250x650')
        self.login.resizable(0, 0)
        self.login.title('Student Teacher Dashboard Login')

        self.login.configure(bg=bg_color)

        login_frame = Frame(self.login, bd=4, borderwidth=4, relief='ridge', bg=highlight_color)
        login_frame.pack(fill='both', expand=True)

        mylabel = Label(login_frame, text="Login", font=('Arial', 16, 'bold'), padx=10, pady=10, bg=highlight_color, fg='black')
        mylabel.grid(row=0, column=0, columnspan=2, sticky="ew")

        stulabel = Label(login_frame, text="Student", font=('Arial', 16, 'bold'), padx=10, pady=10, bg=highlight_color, fg='black')
        stulabel.grid(row=1, column=1, columnspan=2, sticky="ew")
        registernum_label = Label(login_frame, text="Register Number : ", padx=10, pady=10, font=('Arial', 15), bg=highlight_color, fg='black')
        registernum_label.grid(row=2, column=0)
        self.registernum_entry = Entry(login_frame, width=30)
        self.registernum_entry.grid(row=2, column=1)

        password_label = Label(login_frame, text="Password : ", padx=10, pady=10, font=('Arial', 15), bg=highlight_color, fg='black')
        password_label.grid(row=3, column=0)
        self.password_entry = Entry(login_frame, width=30, show="*")
        self.password_entry.grid(row=3, column=1)

        register_button = Button(login_frame, text="Login", font=('Arial', 15), command=self.checkValues, padx=10, pady=10, bd=2, borderwidth=2, bg=button_color, fg='white')
        register_button.grid(row=4, column=0, columnspan=2)

        faclabel = Label(login_frame, text="Faculty", font=('Arial', 16, 'bold'), padx=10, pady=10, bg=highlight_color, fg='black')
        faclabel.grid(row=1, column=5, columnspan=2, sticky="ew")
        
        email_label = Label(login_frame, text="Email : ", padx=10, pady=10, font=('Arial', 15), bg=highlight_color, fg='black')
        email_label.grid(row=2, column=4)
        self.email_entry = Entry(login_frame, width=30)
        self.email_entry.grid(row=2, column=5)

        password_label = Label(login_frame, text="Password : ", padx=10, pady=10, font=('Arial', 15), bg=highlight_color, fg='black')
        password_label.grid(row=3, column=4)
        self.password_entry_fac = Entry(login_frame, width=30, show="*")
        self.password_entry_fac.grid(row=3, column=5)

        register_button = Button(login_frame, text="Login", font=('Arial', 15), command=self.checkfacValues, padx=10, pady=10, bd=2, borderwidth=2, bg=button_color, fg='white')
        register_button.grid(row=4, column=4, columnspan=2)
...
class Register:
    def __init__(self) -> None:
        self.register = Tk()
        self.register.geometry('1250x650')
...
class Home:
    def __init__(self, stuORfac, data) -> None:
        ...
        profile_button.pack(side=LEFT, padx=60, pady=5)
        ...
        search=Entry(top_frame, width=50)
        search.pack(side=LEFT, padx=10, pady=5)
        search_button=Button(top_frame,text='🔍')
        search_button.pack(side=LEFT, padx=5, pady=5)
        canvas = Canvas(home_frame, bg=highlight_color, width=1000, height=500)
        ...
        try:
            ...
            for regno, image_data, description, username in images:
                ...
                img_label.pack(side=TOP,anchor=CENTER, padx=10, pady=10)
